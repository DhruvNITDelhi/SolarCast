...
class ForecastRequest(BaseModel):
    ...
    tilt: Optional[float] = Field(
        None, ge=0, le=90, description="Panel tilt in degrees (default: latitude)"
    )
    azimuth: Optional[float] = Field(
        None,
        ge=0,
        le=360,
        description="Panel azimuth in degrees (default: 180 for India)",
    )
    ...
    forecast_hours: int = Field(24, ge=24, le=72, description="Forecast horizon in hours")
...
class TimeSeriesData(BaseModel):
    ...
    irradiance: float = Field(..., description="Plane-of-array irradiance in W/m2")
    ghi: float = Field(..., description="Global Horizontal Irradiance in W/m2")
...
class DailySummary(BaseModel):
    """Daily generation rollup within the forecast window."""
